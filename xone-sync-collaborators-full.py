...
def send_to_xone_api(users_data: List[Dict], dry_run: bool = True) -> Dict:
    """
    Envia os dados dos usuários para a API do XoneCloud em uma única chamada
    (payload com a lista completa de usuários).
    """
    ...
    if dry_run:
        ...
        print(f"Total de usuários: {len(users_data)} (chamada única)")
        print(f"Token: {XONE_API_TOKEN[:30]}...")
        print(f"Primeiro payload: {json.dumps(payload[0], ensure_ascii=False)}")
        return {"status": "dry_run", "users_count": len(users_data)}

    print(f"\nEnviando {len(users_data)} usuários para XoneCloud API em chamada única...")
    print(f"URL: {XONE_API_URL}\n")
    ...
    try:
        ...
        msg = f"Erro na chamada única: {str(e)[:200]}"
        print(msg)
        return {"status": "error", "error": msg}

    print(f"Resposta HTTP {response.status_code}: {response.text[:300]}")
    return {"status": "success", "successful": len(users_data)}
